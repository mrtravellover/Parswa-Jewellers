...
module.exports = async (req, res) => {
  ...
  if (!memCache.customMapping) {
    memCache.customMapping = (await firestoreGet('liveMarket/ratnam_customMapping')) || {};
  }
  ...
  if (!memCache.mapping) {
    memCache.mapping = (await firestoreGet('liveMarket/ratnam_instrumentMapping')) || {};
  }
  if (!memCache.unknown) {
    const unknownDoc = await firestoreGet('liveMarket/ratnam_unknownInstruments');
  ...
  if (mappingChanged) {
    firestoreSet('liveMarket/ratnam_instrumentMapping', mapping).catch(() => {});
  }
  if (unknownChanged) {
    firestoreSet('liveMarket/ratnam_unknownInstruments', { items: unknownList }).catch(() => {});
  }
