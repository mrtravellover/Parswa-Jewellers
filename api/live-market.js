...
// Polls Ratnam Gold's HTTP feed fresh on every request, parses the
...
// ONE Firestore document (liveMarket/ratnam_data) instead of two separate
...
const UPSTREAM_URL =
  'https://bcast.ratnamgold.in:7768/VOTSBroadcastStreaming/Services/xml/GetLiveRateByTemplateID/ratnam';
const DATA_PATH = 'liveMarket/ratnam_data';
const CUSTOM_MAPPING_PATH = 'liveMarket/ratnam_customMapping';
  ...
  for (const row of rows) {
    ...
    if (known) {
      if (String(known.id) !== String(row.id)) {
        rolloverEvents.push({ source: 'ratnam', name: row.rawName, oldId: known.id, newId: row.id, detectedAt: now });
