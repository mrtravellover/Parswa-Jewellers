...
// Polls KJ Bullion's HTTP feed fresh on every request, parses the
...
// network round trip) rather than Firestore on every single request.
//
// Cold-start reads: "mapping" and "unknown instruments" now live together in
// ONE Firestore document (liveMarket/kjbullion_data) instead of two separate
// ones — Firestore charges per document read, not per field, so this cuts
// those two reads down to one every time a fresh function instance starts
// (which happens more often than you'd think, since more simultaneous
// visitors means more parallel Vercel instances, each needing its own
// first-time read). customMapping stays in its OWN separate document
// deliberately — it's the one piece written by a DIFFERENT file
// (link-instrument.js / reset-retail.js) from a different function
// entirely, so combining it here would risk one file's write silently
// clobbering the other's fresh change with stale cached data.
//
...
const UPSTREAM_URL =
  'https://bcast.kjbullion.com:7768/VOTSBroadcastStreaming/Services/xml/GetLiveRateByTemplateID/kjbullion';
const DATA_PATH = 'liveMarket/kjbullion_data';
const CUSTOM_MAPPING_PATH = 'liveMarket/kjbullion_customMapping';
...
module.exports = async (req, res) => {
  ...
  // or renamed instrument to a card without any code change. Kept as its
  // own read (own document) — see file header for why.
  if (!memCache.customMapping) {
    memCache.customMapping = (await firestoreGet(CUSTOM_MAPPING_PATH)) || {};
  }
  ...
  // Combined read: mapping + unknown together, one document instead of two.
  if (!memCache.mapping || !memCache.unknown) {
    const data = (await firestoreGet(DATA_PATH)) || {};
    if (!memCache.mapping) memCache.mapping = data.mapping || {};
    if (!memCache.unknown) memCache.unknown = data.unknown || {};
  }
  ...
  for (const row of rows) {
    ...
    if (known) {
      if (String(known.id) !== String(row.id)) {
        rolloverEvents.push({ source: 'kjbullion', name: row.rawName, oldId: known.id, newId: row.id, detectedAt: now });
      ...
      // Already logged as unknown — refresh the price in-memory (visible
      // for as long as this instance stays warm) WITHOUT writing to
      // Firestore every single poll. These prices are only ever used for
      // owner identification in the linking picker, not shown to customers,
      // so writing on every 1-second tick was pure waste.
  ...
  // Only hit Firestore for writes when something actually changed — one
  // combined write covers both mapping and unknown together now. Fire-and-
  // forget (not awaited) — a slow/failed write here shouldn't delay the
  // actual rate response going back to the browser.
  if (stateChanged) {
    firestoreSet(DATA_PATH, { mapping: mapping, unknown: unknownList }).catch(() => {});
  }
