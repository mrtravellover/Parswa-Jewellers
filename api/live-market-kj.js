...
module.exports = async (req, res) => {
  ...
  if (!memCache.customMapping) {
    memCache.customMapping = (await firestoreGet('liveMarket/kjbullion_customMapping')) || {};
  }
  ...
  if (!memCache.mapping) {
    memCache.mapping = (await firestoreGet('liveMarket/kjbullion_instrumentMapping')) || {};
  }
  if (!memCache.unknown) {
    const unknownDoc = await firestoreGet('liveMarket/kjbullion_unknownInstruments');
  ...
  for (const row of rows) {
    ...
    if (known) {
      ...
      // so writing on every 1-second tick was pure waste — and was quietly
      // burning through the daily write quota fast enough to risk disrupting
      // the actual live rate polling, which is the one thing that must
  ...
  if (mappingChanged) {
    firestoreSet('liveMarket/kjbullion_instrumentMapping', mapping).catch(() => {});
  }
  if (unknownChanged) {
    firestoreSet('liveMarket/kjbullion_unknownInstruments', { items: unknownList }).catch(() => {});
  }
