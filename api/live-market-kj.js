...
module.exports = async (req, res) => {
  ...
  if (!memCache.customMapping) {
    const customSnap = await db.collection('liveMarket').doc('kjbullion_customMapping').get();
    memCache.customMapping = customSnap.exists ? customSnap.data() : {};
  }
  ...
  if (!memCache.mapping) {
    const mappingSnap = await db.collection('liveMarket').doc('kjbullion_instrumentMapping').get();
    memCache.mapping = mappingSnap.exists ? mappingSnap.data() : {};
  }
  if (!memCache.unknown) {
    const unknownSnap = await db.collection('liveMarket').doc('kjbullion_unknownInstruments').get();
  ...
  if (mappingChanged) {
    db.collection('liveMarket').doc('kjbullion_instrumentMapping').set(mapping, { merge: true }).catch(() => {});
  }
  if (unknownChanged) {
    db.collection('liveMarket').doc('kjbullion_unknownInstruments').set({ items: unknownList }).catch(() => {});
  }
