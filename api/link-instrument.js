...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Save the link (merge — read current state first, add the new key, write back)
    ...
    const unknownPath = 'liveMarket/' + prefix + 'unknownInstruments';
    const unknownData = await firestoreGet(unknownPath);
    if (unknownData && unknownData.items && unknownData.items[normalized]) {
      delete unknownData.items[normalized];
      await firestoreSet(unknownPath, unknownData);
    }
