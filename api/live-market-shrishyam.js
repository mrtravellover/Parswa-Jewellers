...
module.exports = async (req, res) => {
  ...
  if (!memCache.customMapping) {
    const customSnap = await db.collection('liveMarket').doc('shrishyam_customMapping').get();
  ...
  if (!memCache.mapping) {
    const mappingSnap = await db.collection('liveMarket').doc('shrishyam_instrumentMapping').get();
    memCache.mapping = mappingSnap.exists ? mappingSnap.data() : {};
  }
  if (!memCache.unknown) {
    const unknownSnap = await db.collection('liveMarket').doc('shrishyam_unknownInstruments').get();
  ...
  if (mappingChanged) {
    db.collection('liveMarket').doc('shrishyam_instrumentMapping').set(mapping, { merge: true }).catch(() => {});
  }
  if (unknownChanged) {
    db.collection('liveMarket').doc('shrishyam_unknownInstruments').set({ items: unknownList }).catch(() => {});
  }
